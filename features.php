...
if ($where_str == "") {$sql = "select id, name, floor, type, label, wheelchair_access, accessible_sink, sharps, lactation, changing_table, braille_sign, shower, private from building natural join bathroom order by name";}
	else {$sql = "SELECT id, name, floor, type, label, wheelchair_access, accessible_sink, sharps, lactation, changing_table, braille_sign, shower, private from building natural join bathroom where $where_str order by name";}
